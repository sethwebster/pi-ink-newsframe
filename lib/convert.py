...
class convert():
  ...
  def resize(self, source_file, dest_file, display_height, display_width, fill=False):
    command_template =  'convert {source} -resize {height}x{width} -extent {height}x{width}  -colorspace gray +dither -colors 16 -gravity center -alpha remove -rotate 90 {destination}'
    if (fill):
